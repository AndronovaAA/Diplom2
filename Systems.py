...
def Random_System(size_x, size_u, size_y, size_l, F1 = True, F2 = True, E1 = True, E2 = True):
    ...
    if F2 == True:
        F_2 = 0.3 * np.random.randn(size_y, size_x)
    else:
        F_2 = np.zeros((size_y,size_x))

    if F1 == True:
        F_1 = 0.3 * np.random.randn(size_x, size_x)
    else:
        F_1 = np.zeros((size_x, size_x))

    if E1 == True:
        E_1 = 0.3 * np.random.randn(size_x, size_x)
    else:
        E_1 = np.zeros((size_x, size_x))

    if E2 == True:
        E_2 = 0.3 * np.random.randn(size_x, size_u)
    else:
        E_2 = np.zeros((size_x, size_u))
